...
struct rtdevice_pwm_device pwm_device_list[] =
    {
#ifdef BSP_USING_T1_PWM
        {
            .periph = TIM1,
            .name = "pwm1",
#ifdef BSP_USING_T1_PWM1
...
static rt_err_t ch32v3_pwm_device_enable(struct rt_device_pwm *device, struct rt_pwm_configuration *configuration, rt_bool_t enable)
{
...
static rt_err_t ch32v3_pwm_device_get(struct rt_device_pwm *device, struct rt_pwm_configuration *configuration)
{
  struct rtdevice_pwm_device *pwm_device;
  ...
  tim_clock = ch32v3_tim_clock_get(pwm_device->periph);
  channel_index = configuration->channel;
...
static rt_err_t ch32v3_pwm_device_set(struct rt_device_pwm *device, struct rt_pwm_configuration *configuration)
{
  struct rtdevice_pwm_device *pwm_device;
  rt_uint32_t arr_counter, ccr_counter, prescaler, sample_freq;
  rt_uint32_t channel_index;
...
static rt_err_t drv_pwm_control(struct rt_device_pwm *device, int cmd, void *arg)
{
  ...
  {
  case PWM_CMD_ENABLE:
    return ch32v3_pwm_device_enable(device, configuration, RT_TRUE);
  case PWM_CMD_DISABLE:
    return ch32v3_pwm_device_enable(device, configuration, RT_FALSE);
  case PWM_CMD_SET:
    return ch32v3_pwm_device_set(device, configuration);
  case PWM_CMD_GET:
    return ch32v3_pwm_device_get(device, configuration);
  default:
    return RT_EINVAL;
  }
}
...
static int rt_hw_pwm_init(void)
{
  ...
  {
    ch32v3_tim_clock_init(pwm_device_list[index].periph);
    for (channel_index = 0; channel_index < sizeof(pwm_device_list[index].channel); channel_index++)
    {
      ...
      {
        ch32v3_pwm_io_init(pwm_device_list[index].periph, pwm_device_list[index].channel[channel_index]);
      }
    }
